...
public:
  size_t send(const char *buf, size_t size) {
    const auto ec =
        m_process.write(reinterpret_cast<const uint8_t *>(buf), size);
    if (ec) {
      throw std::runtime_error(ec.message());
    }
    return size;
...
void libnvc::reproc_device::spawn(char *argv[]) { m_impl->spawn(argv); }

void libnvc::reproc_device::kill() { m_impl->kill(); }
