...
private:
  // static size_t read_iodev(mpack_tree_t *ptree, char *buf, size_t count) {
    ...
    // throw std::runtime_error(
        // fmt::format(": Empty context in mpack_tree_t: %p", ptree));
  // }
};
}
...
// void inline check_node_type(mpack_node_t node, mpack_type_t expected_type) {
  ...
  // if (true && (type == mpack_type_int || type == mpack_type_uint) &&
  ...
  // if (type != expected_type) {
    // throw std::runtime_error(fmt::format(
        // ": Node type doesn't match: %s, expecting: %s",
        // mpack_type_to_string(type), mpack_type_to_string(expected_type)));
  // }
// }
...
// mp_read<std::array<int64_t, 2>>(mpack_node_t node) {
  // check_node_type(node, mpack_type_array);
  // size_t length = mpack_node_array_length(node);
  // if (length != 2) {
    // throw std::runtime_error(fmt::format(
        // ": Try to read std::array<int64_t, 2> while mpack array size is %zu",
        // length));
  // }
...
// inline std::map<K, V> mp_read_map(mpack_node_t node) {
...
// mp_read<std::vector<std::string>>(mpack_node_t node) {
  // return mp_read_array<std::string>(node);
// }

// template <>
// inline std::vector<libnvc::object>
// mp_read<std::vector<libnvc::object>>(mpack_node_t node) {
  // return mp_read_array<libnvc::object>(node);
// }

// template <>
// inline std::vector<int64_t> mp_read<std::vector<int64_t>>(mpack_node_t node) {
  // return mp_read_array<int64_t>(node);
// }

// template <>
// inline std::vector<std::map<std::string, libnvc::object>>
// mp_read<std::vector<std::map<std::string, libnvc::object>>>(mpack_node_t node) {
  // return mp_read_array<std::map<std::string, libnvc::object>>(node);
// }

// template <>
// inline std::map<std::string, libnvc::object>
// mp_read<std::map<std::string, libnvc::object>>(mpack_node_t node) {
...
// template <> inline libnvc::object mp_read<libnvc::object>(mpack_node_t node) {
  ...
  // default: {
    // throw std::runtime_error(
        // fmt::format(": Unsupport type: %s", mpack_type_to_string(type)));
  // }
  // }
// }
}
...
libnvc::api_client::api_client(/*libnvc::io_device *piodev*/)
    // : m_decoder(std::make_unique<stream_decoder>(piodev)), m_seqid(1),
      // m_iodev(piodev), m_onresp(), m_onresperr() 
			{
  const char *lib_signature = "{{build_signature}}";
  if (std::strcmp(lib_signature, build_signature()) != 0) {
    throw std::runtime_error(fmt::format(
        ": Using incompatible header and lib: header = %s, lib = %s",
        build_signature(), lib_signature));
  }
}
...
// static void log_server_pack_node(mpack_node_t node) {
  ...
  // }
  // libnvc::log(libnvc::LOG_INFO, fmt::format(": %s", log_str.c_str()).c_str());
// }
...
// template <size_t reqid>
// libnvc::resp_variant inn_make_resp_variant([[maybe_unused]] mpack_node_t node) {
  ...
  // } else {
    ...
    // //
    // // here initialization of x will choose bool, and the std::get<> triggers
    // // exception...
    // return libnvc::resp_variant(
        // mp_read<typename libnvc::req<reqid>::resp_type>(node));
  // }
// }

// static void inn_dispatch_req(
    // size_t reqid, int64_t msgid, mpack_node_t node,
    // std::map<int64_t, std::function<void(libnvc::resp_variant)>> &resp_pool) {
...
// {% endfor %}
    // // clang-format on
  // default: {
    // throw std::invalid_argument(fmt::format(": Invalid reqid: %zu", reqid));
  // }
  // }
// }

// static void inn_dispatch_notif(const char *notif_name, mpack_node_t event_node,
                               // libnvc::api_client *pclient) {
  // // not argument check
  ...
  // if (notif_name == nullptr) {
    // throw std::invalid_argument(": Invalid notif_name: (nullptr)");
  // }
...
// {% else %}
        ...
        // if(notif_parms_count != array_length){
            // throw std::runtime_error(fmt::format(": Incorrect parameter count: %zu, expecting %zu", array_length, notif_parms_count));
        // }
...
// {% endfor %}
  ...
  // } else {
    // throw std::runtime_error(
        // fmt::format(": Get unknown notification: %s", notif_name));
  // }
// }
...
int64_t libnvc::api_client::poll_one() {
  ...
  // if (auto root_type = mpack_node_type(rootopt.value());
      // root_type != mpack_type_array) {
    // throw std::runtime_error(
        // fmt::format(": The msgpack from nvim server is not an array: %s",
                      // mpack_type_to_string(root_type)));
  // }

  // if (auto root_size = mpack_node_array_length(rootopt.value());
      // root_size < 1) {
    // throw std::runtime_error("The msgpack from nvim server is an empty array");
  // }

  // if (auto first_node_type =
          // mpack_node_type(mpack_node_array_at(rootopt.value(), 0));
      // first_node_type != mpack_type_uint) {
    // throw std::runtime_error(
        // fmt::format(": The msgpack from nvim server has first node type %s, "
                      // "expect mpack_type_uint",
                      // mpack_type_to_string(first_node_type)));
  // }
  ...
  // }
  // case libnvc::RESP: {
    // if (size_t array_size = mpack_node_array_length(rootopt.value());
        // array_size != 4) {
      // throw std::runtime_error(
          // fmt::format(": RESP array size is not 4: %zu", array_size));
    // }

    // int64_t msgid = mpack_node_i64(mpack_node_array_at(rootopt.value(), 1));
    // auto [req_id, seq_id] = msgid_decomp(msgid);
    // libnvc::log(libnvc::LOG_INFO,
                // fmt::format(": req_id = %s, seq_id = %" PRIi64,
                              // libnvc::idstr(req_id), seq_id)
                    // .c_str());

    // if (libnvc::idstr(req_id) == nullptr) {
      // throw std::runtime_error(
          // fmt::format(": RESP to invalid REQ: %" PRIi64, req_id));
    // }

    // if (seq_id <= 0) {
      // throw std::runtime_error(
          // fmt::format(": RESP with invalid seq_id: %" PRIi64, seq_id));
    // }

    // if (auto errnode = mpack_node_array_at(rootopt.value(), 2);
        // !mpack_node_is_nil(errnode)) {
      // if (size_t errnode_size = mpack_node_array_length(errnode);
          // errnode_size != 2) {
        // throw std::runtime_error(fmt::format(
            // ": RESP error array [type, message] has invalid size: %zu",
            // errnode_size));
      // }
    ...
    // if (m_onresp.find(msgid) == m_onresp.end()) {
      // if (m_onresperr.find(msgid) != m_onresperr.end()) {
        // throw std::runtime_error(fmt::format(
            // ": Found error handler but no resp handler: %zu", req_id));
      // }
      // m_onresperr.erase(msgid);
      // return 0;
    // }

    // inn_dispatch_req(req_id, msgid, mpack_node_array_at(rootopt.value(), 3),
                     // m_onresp);
    // return msgid;
  // }
  // case libnvc::NOTIF: {
    ...
    // if (redraw_node_name != "redraw") {
      // throw std::runtime_error(fmt::format(": Get unknown node string: %s",
                                             // redraw_node_name.c_str()));
    // }
    ...
    // for (size_t group_index = 0; group_index < event_group_length;
         // ++group_index) {
      ...
      // for (size_t event_index = 1; event_index < event_array_length;
           // ++event_index) {
        // inn_dispatch_notif(event_name.c_str(),
                           // mpack_node_array_at(event_array, event_index), this);
      // }
    // }
    // return 0;
  // }
  // default: {
    // throw std::runtime_error(
        // fmt::format(": The msgpack from nvim server has message type: %d, "
                      // "expect [REQ, RESP, NOTIF]: %d",
                      // (int)(msg_type)));
  // }
  // }
}
