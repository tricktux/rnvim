...
// clang-format on
    ...
    constexpr const char *idstr(size_t id) {
      if (id == 0 ||
          id >= std::extent<decltype(g_nvim_api_reserved_string_tbl)>::value) {
        return nullptr;
      }
    ...
    constexpr const char *reqstr(size_t id) {
      ...
      if (auto str = g_nvim_api_reserved_string_tbl[id];
          libnvc::ctf::strncmp(str, "req::", 5) == 0) {
        return str + 5;
      }
      return nullptr;
    }
    ...
    constexpr size_t strid(const char *str) {
      ...
      for (size_t index = 1;
           index < std::extent<decltype(g_nvim_api_reserved_string_tbl)>::value;
           ++index) {
        if (libnvc::ctf::strcmp(str, g_nvim_api_reserved_string_tbl[index]) ==
            0) {
          return index;
        }
      }
    ...
    constexpr size_t max_reqid() {
      for (size_t id =
               std::extent<decltype(g_nvim_api_reserved_string_tbl)>::value - 1;
           libnvc::idstr(id) != nullptr; --id) {
        if (libnvc::ctf::strncmp(libnvc::idstr(id), "req::", 5) == 0) {
          return id;
        }
      }
    ...
    public:
      virtual size_t
      send(const char *,
           size_t) = 0; // blocking send, return after done all bytes
    ...
    // using helper class object_wrapper since  we can't define recursive type
    // in cpp
    ...
    public:
      libnvc::object &ref() { return *(m_ptr.get()); }
...
{% endfor %}
    ...
    inline int64_t msgid(size_t req_id, int64_t seq_id) {
      ...
      return ((seq_id & 0x0000ffffffffffff) << 16) |
             ((int64_t)(req_id)&0x000000000000ffff);
    }
    ...
    private:
      ...
      // there is no official way to track if the writer called
    ...
    public:
      ...
      std::string_view pack_req(int64_t msgid, Args... args) {
...
{% endfor %}
    ...
    private:
      const char *build_signature() const { return "{{build_signature}}"; }

    public:
      int64_t seqid() const { return m_seqid; }

    private:
      void add_seqid(int64_t add) { m_seqid += add; }
    ...
    public:
      template <size_t reqid, typename on_resp_t>
      inline int64_t
      forward(const typename libnvc::req<reqid>::parms_tuple &parms,
              on_resp_t on_resp) {
        add_seqid(1);
        ...
        m_iodev->send(
            std::apply(fn_pack, std::tuple_cat(std::make_tuple(msgid), parms)));
        regcb_resp<reqid, on_resp_t>(on_resp);
        return msgid;
      }

      template <size_t reqid, typename on_resp_t, typename on_resperr_t>
      inline int64_t
      forward(const typename libnvc::req<reqid>::parms_tuple &parms,
              on_resp_t on_resp, on_resperr_t on_resperr) {
        add_seqid(1);
        auto msgid = libnvc::msgid(reqid, seqid());
...
{% endfor %}
    ...
    public:
      void clear() { m_utf8code = INVALID_UTF8; }

      void set(uint32_t, int);

    public:
      bool valid() const { return m_utf8code != INVALID_UTF8; }

    public:
      size_t wc_width() const { return m_wcwidth; }
    ...
    public:
      board(size_t grid_width, size_t grid_height)
          : m_grid_width(grid_width), m_grid_height(grid_height), m_cursor_x(0),
            m_cursor_y(0), m_cells(grid_width * grid_height) {}
    ...
    public:
      size_t cursor_x() const { return m_cursor_x; }

      size_t cursor_y() const { return m_cursor_y; }

      size_t width() const { return m_grid_width; }

      size_t height() const { return m_grid_height; }

    private:
      size_t cursor_off() const { return cursor_y() * width() + cursor_x(); }
    ...
    public:
      void advance_cursor(size_t pos) { set_cursor(cursor_off() + pos); }
    ...
    private:
      CELL &get_cell(size_t x, size_t y) { return m_cells.at(y * width() + x); }
    ...
    private:
      CELL &get_cell() { return m_cells.at(cursor_off()); }

    public:
      const CELL &get_cell() const { return m_cells.at(cursor_off()); }
    };
    ...
    public:
      struct hl_attrdef {
        ...
        void clear() { std::memset(this, 0, sizeof(hl_attrdef)); }
      };
    ...
    private:
      void flush_board() { m_backboard = m_currboard->clone(); }

    public:
      size_t cursor_x() const { return m_backboard->cursor_x(); }

      size_t cursor_y() const { return m_backboard->cursor_y(); }

    public:
      size_t width() const { return m_currboard->width(); }

      size_t height() const { return m_currboard->height(); }
    ...
    public:
      void on_default_colors_set(int64_t rgb_fg, int64_t rgb_bg, int64_t rgb_sp,
                                 int64_t, int64_t) {
        hl_attrdef hldef;

        // currently nvim always gives fg, bg, sp as -1
        // -1 means no color set yet, nvim hasn't implement its way to choose a
        // proper default color
    ...
    public:
      void on_grid_scroll(int64_t, int64_t, int64_t, int64_t, int64_t, int64_t,
                          int64_t);

      void on_grid_clear(int64_t) { m_currboard->clear_char(); }
    ...
    public:
      void on_hl_attr_define(int64_t,
                             const std::map<std::string, libnvc::object> &,
                             const std::map<std::string, libnvc::object> &,
                             const std::vector<libnvc::object> &);

    public:
      void on_grid_line(int64_t, int64_t, int64_t,
                        const std::vector<libnvc::object> &);
    ...
    public:
      const auto &get_modedef() const { return m_modedefs.at(m_mode); }
    };
    ...
    public:
      size_t width() const { return m_client->width(); }

      size_t height() const { return m_client->height(); }

    public:
      size_t cell_width() const { return m_cell_width; }

      size_t cell_height() const { return m_cell_height; }

    public:
      virtual void draw_ex(int,  // dst_x
                           int,  // dst_y
                           int,  // src_x
                           int,  // src_y
                           int,  // src_w
    ...
    public:
      virtual void draw_char(const struct libnvc_rect *, // box_draw
